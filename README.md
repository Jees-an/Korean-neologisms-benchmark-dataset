...
| 2  | `최초 출현 연도` | `2023`              | 해당 신어가 최초로 출현한 연도(<네이버 뉴스> 기준) |
...
| 10 | `단어 형성법`    | `혼성`     | 단일어 및 복합어(합성어, 파생어, 혼성어, 축약어)의 단어 형성법 |
| 11 | `전문 분야`      | `『경제』`           | 신어가 출현한 전문 분야 |
| 12 | `의미 범주`      | `〔경제생활〕`       | 신어가 빈번하게 사용되는 의미 범주 |
| 13 | `뜻풀이`         | `재미를 추구하는 재테크. ‘재미’와 ‘재테크’가 결합한 말로, 금융 플랫폼에서 캐릭터를 공유하여 임무를 수행하거나 지인과 함께 포인트를 받는 과정을 재미있게 느끼면서 재테크를 하는 것을 말한다.` | 신어에 대한 정의문과 설명문 |
| 14 | `용례`           | `¶OO 은행은 엠제트 세대에게 다가가기 위해, 혜택과 함께 재미를 추구하는 {잼테크} 마케팅을 실시하고 있다.` | 신어가 형성 및 사용되는 전형적인 문맥 |
